...
#### Where the memory goes matters more than how much there is

The upstream measurements point at a sharper question than "resident or streamed".
At a fixed total budget, moving memory from the expert cache to the dense trunk
was **1.69× faster**, and the expert cache contributed nothing measurable until
its arena passed roughly 36 GB.

The uncomfortable part is that the fastest configuration read **79% more** expert
bytes than the slowest, at a cache hit rate of zero, and still won by 69%.
Optimizing cache hit rate — the number anyone would naturally optimize — leads
to the slower system.

> **Allocation beats capacity. Measure where the bytes live, not how many are cached.**

So the comparison above should hold total memory fixed and vary only the split,
rather than varying the total. A sweep that changes both cannot attribute the
result to either.

#### What has been checked locally, and what has not
