...
### What has to move between them: 28 KB

The hidden state is 7168 values at 4 bytes. **28 KB per layer boundary, 2.67 MB end to
end.** Against 2.24 GB of local reading per layer, that is a ratio of about **1 : 80,000**.

This is the part people expect to be the problem and it is not. Ordinary ethernet carries
it. A 1 Gbit/s port sustains roughly **4,360 tokens per second** of hop traffic — far more
than the cards at either end can consume. No NVLink, no InfiniBand, no special fabric.

That is the whole difference between this shape and tensor parallelism. Tensor
parallelism splits a single layer across devices and must reconcile partial results every
layer, which is why it needs 900 GB/s between cards. Splitting *by layer* moves only the
thing that was going to move anyway.
...
| interconnect | n/a | 28 KB per hop, 1 : 80,000 against local traffic |
...
- **Expert identity is data-dependent.** Layer L's router reads layer L's input, so the
  16 experts become known microseconds before they are needed. We tested three ways to
  predict them in advance and two were dead on arrival; the only real structure is
  temporal reuse of 40–53%, which the existing cache already captures almost perfectly.
...
> needs more than 1.45 GB resident at once per layer, and the only thing that has to cross
> between layers is 28 KB. Those two measured facts are what make many small machines a
> real option where one large one is not.**
